...
def backtest_strategy(
    ...
    stop_loss=0.01,
    hold_bars=5,
    rsi_buy_thresh=40,    # Alış sinyali için RSI eşiği (esnek)
    macd_buy_thresh=0     # Alış sinyali için MACD eşiği (esnek)
):
    ...
    for i in range(lookback, len(closes) - hold_bars):
        try:
            current_slice = {
                ...
                'high': highs[i - lookback:i],
                'low': lows[i - lookback:i],
            ...
            continue  # Göstergede hata varsa atla

        buy_condition = (rsi_val < rsi_buy_thresh) and (macd_line > macd_buy_thresh)
        if buy_condition:
            entry = closes[i]
...
async def main_loop():
    ...
    while True:
        ...
        )from rapor_mesaj import create_report_message  # Eğer farklı isimdeyse dosya adını değiştir

msg = create_report_message(
    kisavadeli_analiz_bolumu,
    rapor_1h,
    rapor_4h,
    rapor_1d,
